...
app.delete('/api/users/:id', (req, res) => {
  ...
  const removedPostIds = new Set(db.posts.filter((p) => p.userId === userId).map((p) => p.id));
  db.posts = db.posts.filter((p) => p.userId !== userId);

  // Clean likes on remaining posts
  db.posts = db.posts.map((p) => ({ ...p, likes: p.likes.filter((id) => id !== userId) }));

  // Remove comments authored by the user or on deleted posts
  db.comments = db.comments.filter((c) => c.userId !== userId && !removedPostIds.has(c.postId));

  // Clean follower/following references
...
app.delete('/api/posts/:id', (req, res) => {
  ...
  // Remove comments on that post
  db.comments = db.comments.filter((c) => c.postId !== postId);

  saveDB(db);
  res.json({ deletedId: postId });
});
...
// SPA fallback to index.html for frontend routes
app.get('*', (_req, res) => {
